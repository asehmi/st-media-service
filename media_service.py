...
class MediaService():
    ...
    def __init__(self):
        ...
        if os.path.isfile('media_service.toml'):
            service_settings = toml.load('media_service.toml')
        else:
            service_settings = toml.load('media_service.example.toml')
    ...
    def _rename_file_with_prefix(self, source: str, media_file: str, prefix: str):
        ...
        if not os.path.isfile(src):
            raise FileNotFoundError({'msg': src, 'status_code': 404})
        
        try:
            os.rename(src, dest)
            print("Renamed:", src, 'to', dest)
        except Exception as e:
            return Exception({'msg': str(e), 'status_code': 404})
